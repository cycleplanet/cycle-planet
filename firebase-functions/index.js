  ...
  ref.once("value", function (snapshot) {
    ...
    if (userRecord.token) {
      ...
      functions.logger.log(
        "req.query user data email registrationToken",
        registrationToken
      );
...
exports.computeCountryMarkerCounts = functions.pubsub
  .schedule("0 */12 * * *")
  .onRun(async (context) => {
    ...
    await Promise.all(
      users.map(async (user) => {
        ...
        if (
          !(user.coordinates && user.coordinates.lat && user.coordinates.lng)
        ) {
          console.log(`Skipping user ${user.id} because no coords`);
          return;
        }

        const hostCountryCode = await geocoder.reverseGeocodeToCountryCode(
          user.coordinates.lat,
          user.coordinates.lng
        );

        if (hostCountryCode) {
          const countryName = countryConstants.getCountryData(hostCountryCode)
            .fullName;
          countMarker("hosts", countryName);
          console.log(`Reverse geocoded user to ${countryName}`);
        } else {
          console.log(
            `Skipping user ${user.id} because no country code for coords`
          );
        }
      })
    );
    ...
    Object.keys(countryMarkerCounts).forEach((cc) => {
      db.ref(`CountryMarkerCounts/${cc}/hosts`)
        ...
        .catch((err) =>
          console.error("Could not write host count for country", err)
        );
      db.ref(`CountryMarkerCounts/${cc}/poi`)
        .set(countryMarkerCounts[cc].poi)
        .catch((err) =>
          console.error("Could not write POI count for country", err)
        );
    });
...
exports.updateMarkerStats = functions.pubsub
  .schedule("0 */12 * * *")
  .onRun((context) => {
    console.log("basTestFunction started");
    let d = new Date().toISOString();
