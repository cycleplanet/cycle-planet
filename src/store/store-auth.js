...
const actions = {
      ...
      .then(() => {
        ...
        if (user && user.emailVerified === false) {
          ...
          const idx = Math.floor(
            Math.random() * Object.keys(state.backgroundimageurl).length
          );
          let timeStamp = Date.now();
          ...
          firebase.db.ref("Users/" + userId).set({
            ...
            imageurl:
              "https://firebasestorage.googleapis.com/v0/b/cycle-planet-292f5.appspot.com/o/placeholder%2Fplaceholder_profile.jpg?alt=media&token=898d4a24-3791-4baf-8b93-4aaadbd4a94a",
            hosting: {
              bike_inside: false,
              max_guests: 0,
              pets_allowed: false,
    ...
    firebase.auth
      ...
      .catch(function (error) {
        ...
        Notify.create({
          ...
          timeout: 2000,
          message:
            "Something went wrong, please contact us to solve the problem.",
        });
      });
  },
  ...
  setFollow({}, payload) {
    ...
    let ref = firebase.db.ref(
      "Follow/" + payload + "/followers/" + userId + "/"
    );
    ref.set(true);
    let ref2 = firebase.db.ref(
      "Follow/" + userId + "/following/" + payload + "/"
    );
    ref2.set(true);
  },
    ...
    user
      ...
      .then(function () {
        ...
        Notify.create({
          ...
          timeout: 7000,
          message:
            "Your email address is updated! A verification email has been send. You will now be logged out.",
        });
        firebase.auth.signOut();
