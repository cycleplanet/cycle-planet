...
const actions = {
  ...
  setItemActionFs2({}, payload) {
    firebase.fs
      ...
      .set(Object.assign({}, payload.data))
      .then(() => {})
      .catch((error) => {});
  },
  ...
  selectGearUser({}, payload) {
    ...
    let newuid = uid();
    firebase.db
      .ref("GearUser/" + payload.pathGear + "/users/" + userId)
      .set(true);
    firebase.db
      .ref(
        "Users/" + userId + "/gear_list_new/" + payload.pathUser + "/" + newuid
      )
      .set(payload.string);
  },
  deleteGearUser({ commit, dispatch }, payload) {
    let userId = firebase.auth.currentUser.uid;
    firebase.db
      .ref("Users/" + userId + "/gear_list_new/" + payload.path)
      .remove();
    dispatch(
      "auth/deleteUserDetailAction",
      userId + "/gear_list_new/" + payload.path,
      { root: true }
    );
  },
  ...
  likeGearItem({}, payload) {
    ...
    let ref = firebase.db.ref(
      "Gear/" + payload.sectionId + "/" + payload.itemId + "/likes/" + userId
    );
...
const getters = {
  ...
  gearItemsSortedByLikes: (state) => {
    ...
    keysOrdered.sort((a, b) => {
      let AProp = state.gear[a].likes
        ? Object.keys(state.countryData.Video[a].likes).length
        : 0;
      let BProp = state.countryData.Video[b].likes
        ? Object.keys(state.countryData.Video[b].likes).length
        : 0;
