...
export default {
	...
	computed: { 
		...
		admin (){
			if(this.myUserDetails){
				if(this.myUserDetails.role==='admin'||this.myUserDetails.role==='team'||this.myUserId==='ieMGYdJeX6ZyKgKeeGhMXPFZkax2'){
					return true
				}else{return false}
			}else{return false}
		},
		teamMember (){
			if(this.myUserDetails){
				if(this.myUserDetails.role==='team'||this.myUserDetails.role==='admin'){
					return true
				}else{
					return false
				}
		...
		roleEditor (){
			if(this.myUserDetails){
				if(this.myUserDetails.role==='editor'||this.myUserDetails.role==='team'||this.myUserDetails.role==='admin'){
					return true
				}else{
					return false
				}
		...
		screenWidth(){let screenWidth = screen.width
			return screenWidth},
		screenHeight(){return screen.height},
		screenInnerHeight(){return window.innerHeight},
		screenViewHeight(){return view.window?.windowScene?.statusBarManager?.statusBarFrame.height??0},
	...
	components: {
		...
		'nice-date-short':		require('components/Shared/Modals/NiceDateShort.vue').default,
		'nice-date2': 			require('components/Shared/Modals/NiceDate2.vue').default,
		'modal-countrychip':	require('components/Shared/Modals/ModalCountryChip.vue').default,
		'modal-countrychip2':	require('components/Shared/Modals/ModalCountryChip2.vue').default,
		'modal-countrychip3':	require('components/Shared/Modals/ModalCountryChip3.vue').default,
		'modal-countrychip4':	require('components/Shared/Modals/ModalCountryChip4.vue').default,
		'modal-username': 		require('components/Shared/Modals/ModalUsername.vue').default,
		...
		'mobile-header': 		require('components/Shared/Modals/MobileHeader.vue').default,
