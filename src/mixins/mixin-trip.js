...
export default {
  ...
  computed: {
    ...
    tripActive() {
      if (this.hasEndDate) {
        if (
          this.daysToGo(this.tripDetails.startdate) < 1 &&
          this.daysToGo(this.tripDetails.enddate) > 0
        ) {
          return true;
        } else {
          return false;
    ...
    daysTraveling() {
      if (this.tripActive) {
        ...
        return (
          -this.daysToGo(this.tripDetails.startdate) -
          -this.daysToGo(this.tripDetails.enddate)
        );
      }
    },
    ...
    currentLocation() {
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        new Geoapify(geoapify.apiKey)
          .reverseGeocode(
            this.storyData.coordinates.lat,
            this.storyData.coordinates.lng
          )
          .then((address) => {
  ...
  methods: {
    ...
    ...mapActions("profile", [
      "saveEditStoryAction",
      "saveExistingCountryListAction",
      "deleteExistingCountryListAction",
    ]),
    tripIdRoute() {
