...
<div>
    <div v-for="(gearGroup, id) in gearGroups" :key="id">
      ...
      <div class="q-pa-md" v-for="(gearItem, gearItemIndex) in gearGroup" :key="gearItemIndex">
      ...
      <p class="cp-h2">{{gearItem.title}}</p>
        ...
        <div class="row no-wrap q-gutter-md" >
          <div  v-for="(item,itemId) in gear[gearItemIndex]" :key="itemId">
            <q-card bordered class="my-card" :style="screenwidthbig?'width: 300px;':'width: 200px;' ">

              <q-card-section>
                <q-item-label class="thumbnail" >
                  <q-img   :src="item.image_url" style="width: 100%; max-height:200px"/>
                </q-item-label>
                <q-item-label>
                  <p :class="screenwidthbig?'text-h6':''">{{item.title}}</p> 
                </q-item-label>
                <q-item-label class="text-caption text-grey">
                  {{item.description}}
                </q-item-label>
                  <q-item-label >
                    <q-chip dense class="text-caption text-green bg-green-2 no-margin">from {{item.price}}</q-chip>
                  </q-item-label>
              </q-card-section>
              ...
              <q-card-actions>
                ...
                :class="item.likes?(item.likes[myUserId] ?'text-red' :''):''"
                :icon="item.likes?(item.likes[myUserId] ? 'favorite' : 'favorite_border') : 'favorite_border'" 
                @click="pressLiked(gearItemIndex,itemId,item.likes?(item.likes[myUserId] ? 'disliked' : 'liked'):'liked')"
                />
                {{item.likes ? Object.keys(item.likes).length : 0}}
               
                  <q-btn flat :style="buttonStyle" @click="openUrl(item.url)"  :size="screenwidthbig?'md':'sm'">
                    View on {{item.name}}
                  </q-btn>
