<template>
  <div>
    <div
      v-if="users && Object.keys(userMessagesSortedByDate).length > 0"
      class="bg-white"
    >
      <q-list v-for="(userSorted, key) in userMessagesSortedByDate" :key="key">
          ...
          <q-item-section>
            <div class="row">
              ...
              <date-chat
                ...
                :dateprop="
                  userSorted[
                    Object.keys(userSorted)[Object.keys(userSorted).length - 1]
                  ].timestamp
                "
              />
            </div>
            <div class="row">
              ...
              <q-space></q-space>
              <q-btn
                v-if="unreadchatlistnew ? unreadchatlistnew[key] : false"
                class="q-mt-xs"
                round
                color="red"
                :label="Object.keys(unreadchatlistnew[key]).length"
                size="xs"
              />
            </div>
          </q-item-section>
      ...
      <div v-if="Object.keys(userMessagesSortedByDate).length === 0">
        <q-card class="q-mb-md q-pa-md bg-green-3">
          <p>
            Messages with other users will appear in this list. With messages,
            you can send host requests to other users.
          </p>
        </q-card>
        <q-list>
          <q-item clickable v-ripple to="/chat/RYFRjpH3DfdWPpRfG9MIFBSPzH33">
            ...
            <q-item-section>
              <q-item-label>{{
                users["RYFRjpH3DfdWPpRfG9MIFBSPzH33"].fullname
              }}</q-item-label>
              <q-item-label caption lines="2"
                >Write your first message to Cycle Planet and let us know what
                you think of the platform ;)</q-item-label
              >
            </q-item-section>
          </q-item>
        </q-list>
    ...
    <div v-else class="bg-white absolute-full text-center">
      <div class="constrain">
        <img
          ...
          src="https://firebasestorage.googleapis.com/v0/b/cycle-planet-292f5.appspot.com/o/Admin%2Fvectors%2FMessages.png?alt=media&token=c0c226ee-2aa8-4597-93ee-3a15a9afdc4a"
        />
...
export default {
  ...
  mounted() {
    this.$store.dispatch("chat/firebaseGetUsers").then(
      (response) => {},
      (error) => {}
    );
  },
