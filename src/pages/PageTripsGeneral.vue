...
<div>
    ...
    <div class="row q-ma-sm">
      ...
      <div v-for="(trip, tripKey) in tripsData" :key="tripKey" class="card q-mb-sm col-md-5 col-sm-5 col-xs-12" style="height:300px;" >
        <q-card  class="cursor-pointer cardstyle2" :class="tripKey===hoverKey?'overlayactive':'overlayhidden'" @click="clickedTrip(tripKey)"  :style="'background-image:  linear-gradient(to bottom,rgba(0,0,0, 0.4), rgba(0,0,0, 0.80)),url('+trip.image+'); height:100%; width: 100%;'" @mouseover="hoveroverMethod(tripKey)" @mouseleave="hoverleaveMethod(tripKey)">
            <div class="text-white q-pa-md bottom-left">
                <div class="row q-mb-md" :class="tripKey===hoverKey?'textactive':'texthidden'">
                    <div class="text-bold text-body1 q-mb-md">{{trip.summary}}</div>
                    <div v-for="(countryKey,index) in trip.countries" :key="index" class="row">
                        <div v-if="index<10">{{countryKey}}</div>
                        <div v-if="index<Object.keys(trip.countries).length-1 &&index<9" class="q-mx-xs">-</div> 
                          
                    </div>
                    <div class="q-ml-sm" v-if="Object.keys(trip.countries).length>9"> & {{Object.keys(trip.countries).length-10}} more</div> 
                </div>
    ...
    <q-btn rounded label="Or create your own trip" class="q-ma-sm" :style="buttonStyle" @click="loggedIn?createTripDialog=true:showLoginDialog()"/>
    ...
    return {
        ...
        tripsData:{
            ...
            "tanger-capetown" : {
                ...
                "summary" : "Meet the incredible hospitality that one of the poorest regions in the world has to offer you.",
