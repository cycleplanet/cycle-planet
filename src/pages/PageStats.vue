<template>
  ...
  >
    ...
    <q-btn
      :style="buttonStyle"
      label="update user in country"
      @click="submitUsersInCountry"
      v-if="admin && Object.keys(users_in_country_temp).length"
    />
    <q-btn
      :style="buttonStyle"
      label="update markers in country"
      @click="submitMarkersPerDay"
      v-if="admin && Object.keys(markers_per_day_temp).length"
    />
    ...
    <div class="row q-gutter-md">
      <q-card class="col-12 col-md q-pa-md">
        ...
        <div
          class="text-h5 text-center flex justify-center items-center"
          style="height: 80%;"
        >
      ...
      <q-card class="col-12 col-md q-pa-md">
        ...
        <q-separator class="q-my-sm" />
        <q-chip
          v-for="(number, refKey, index) in statistics.type_of_users"
          :key="index"
        >
      ...
      <q-card
        ...
        v-if="Object.keys(usersSorted).length"
      >
        ...
        <div class="row">
          ...
          <q-chip
            >+{{ Object.keys(usersSorted).length - 16 }} countries more</q-chip
          >
        </div>
      </q-card>
    </div>
    ...
    <div class="row q-gutter-md">
      ...
      <q-card class="col-12 col-md q-pa-md">
        ...
        <q-separator class="q-my-sm" />
        <q-chip
          v-for="(number, refKey) in statistics.type_of_markers"
          :key="refKey"
        >
          <q-avatar>
            <img :src="markerlist[refKey].iconurl" />
      ...
      <q-card class="col-12 col-md q-pa-md">
        ...
        <div class="row">
          <div
            v-for="(number, countrycode, index) in landMarkersSorted"
            :key="index"
          >
            <q-chip v-if="index < 16">
              <q-avatar>
          ...
          <q-chip
            >+{{ Object.keys(landMarkersSorted).length - 16 }} countries
            more</q-chip
          >
        </div>
      </q-card>
      ...
      <div>
        ...
        <div class="row q-gutter-md">
          <q-card class="col-12 col-md q-pa-md">
            ...
            <q-separator class="q-my-sm" />
            <q-expansion-item
              v-for="(userId, index) in statistics.toplist_points_overall"
              :key="user"
            >
              <template v-slot:header>
                ...
                <q-item-section side>
                  <q-badge
                    class="text-black q-pa-sm text-bold"
                    :label="users[userId].points.score"
                  />
                </q-item-section>
              </template>
              <!-- <div>
          ...
          <q-card class="col-12 col-md q-pa-md">
            ...
            <q-separator class="q-my-sm" />
            <q-expansion-item
              v-for="(userId, index) in statistics.toplist_points_giveaway"
              :key="user"
            >
              <template v-slot:header>
                <q-item-section>
                ...
                <q-item-section side>
                  <q-badge
                    class="text-black q-pa-sm text-bold"
                    :label="users[userId].points.score_giveaway"
                  />
                </q-item-section>
              </template>
              <div>
        ...
        <div class="row q-gutter-x-md">
          ...
          <q-card class="col q-pa-md">
            ...
            <q-separator class="q-my-sm" />
            <div class="text-h5 text-center q-my-lg">
              {{ statistics.total_hostrequests_accepted }} ({{
                (statistics.total_hostrequests_accepted /
                  statistics.total_hostrequests) *
                100
              }}%)
...
export default {
  ...
  methods: {
    ...
    totalUsers() {
      ...
      firebase.db
        .ref("Statistics/users_per_day_sum/" + this.statisticsTimeStamp)
        .set(total);
    },
    totalMarkers() {
    ...
    usersInCountry() {
      ...
      userrKeys.forEach((element) => {
        if (this.users[element].coordinates) {
          ...
          Geoapify.reverseGeocodeToCountryCode(coordinates.lat, coordinates.lng)
            .then((cc) => {
    ...
    submitUsersInCountry() {
      ...
      firebase.db
        .ref("Statistics/users_in_country")
        .set(this.users_in_country_temp);
    ...
    submitMarkersPerDay() {
      firebase.db
        .ref("Statistics/markers_per_day")
        .set(this.markers_per_day_temp);
    },
    markersPerDaySum() {
        ...
        firebase.db
          .ref(
            "Users/" +
              ...
              this.landMarkers[element].date_created
          )
          .set(element);
      });
    },
      ...
      userKeys.forEach((element) => {
        if (!this.users[element].points) {
          ...
          if (this.users[element].points.markers_added) {
            let scoreAddedMarkers =
              Object.keys(this.users[element].points.markers_added).length * 10;
            score = score + scoreAddedMarkers;
          }
          if (this.users[element].points.markers_checked) {
            let scoreCheckedMarkers =
              Object.keys(this.users[element].points.markers_checked).length *
              3;
            score = score + scoreCheckedMarkers;
          }
          if (this.users[element].points.markers_liked) {
            let scoreLikedMarkers =
              Object.keys(this.users[element].points.markers_liked).length * 1;
    ...
    topPointsOverall() {
      ...
      if (
        Object.keys(this.user_points_overall).length ===
        Object.keys(this.users).length
      ) {
        console.log(
          "topPointsOverall 3",
          Object.keys(this.user_points_overall)
        );
        let usersSorted = {},
    ...
    calculatePointsGiveAway() {
      ...
      userKeys.forEach((element) => {
        if (!this.users[element].points) {
          ...
          // console.log('calculatePointsOverall 3', score);
          firebase.db
            .ref("Users/" + element + "/points/score_giveaway")
            .set(score);
        }
      });
    },
    topPointsGiveAway() {
      ...
      let userKeys = Object.keys(this.users);
      userKeys.forEach((element) => {
        this.user_points_giveaway[element] = this.users[
          element
        ].points.score_giveaway;
      });

      if (
        Object.keys(this.user_points_giveaway).length ===
        Object.keys(this.users).length
      ) {
        console.log(
          "topPointsOverall 3",
          Object.keys(this.user_points_giveaway)
        );
        let usersSorted = {},
