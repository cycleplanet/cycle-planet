<template>
  <div>
    <div class="q-gutter-sm no-padding">
      ...
      />
      <div v-if="!value">
        Or find coordinates on
        <a
          href="https://www.openstreetmap.org/"
          class="underline cursor-pointer text-blue-8"
          >OpenStreetMap</a
        >
      </div>
      <q-input
        filled
        v-model="payload.coordinates.lat"
        label="Latitude"
        :rules="[(val) => !!val || 'Field is required']"
        placeholder="Use only numbers and a dot(.), for example: 3.6435345"
      />
      <q-input
        filled
        v-model="payload.coordinates.lng"
        label="Longitude"
        :rules="[(val) => !!val || 'Field is required']"
        placeholder="Use only numbers and a dot(.), for example: 3.6435345"
      />
