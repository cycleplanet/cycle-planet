...
<q-list class="full-width">
    <q-item dense class="no-padding no-margin bg-grey-4" v-if="myUserDetails" >
        <q-item-section avatar dense>
            ...
            <q-btn dense @click="comment()" flat round icon="send" size="md" v-if="newComment"/>
        ...
        <q-item-section >
            <q-input dense class="no-padding no-margin"
            :placeholder="'Comment as '+myUserDetails.fullname"
            v-model="newComment" autogrow
    ...
    methods:{
        comment(){
            if(this.loggedIn){
                ...
                firebase.fs.collection('Markers').doc(this.itemKey).update({comments:this.itemData.comments})
            }else{
