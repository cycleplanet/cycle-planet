    ...
    <form @submit.prevent="sendMessage">
      ...
      <!-- {{feedbackData}} -->
        <q-card-section class="">
          <div v-if="feedbackData.from==='me'">You stayed at {{users[feedbackData.receiver].fullname}} on {{feedbackData.dateProposal}}. Please write some feedback about how you experienced your stay with {{users[feedbackData.receiver].fullname}}, so others can know what to expect. This way, we build a reliable community.</div>
          <div v-if="feedbackData.from==='them'">Thanks for hosting {{users[feedbackData.sender].fullname}}. Please write some feedback about how you experienced having {{users[feedbackData.sender].fullname}} as a guest, so others can know what to expect. This way, we build a reliable community.</div>
          <!-- {{myName}} -->
              <q-select outlined filled class="q-mt-md"
              style="width:250px"
              v-model="experience"
              use-input
              input-debounce="0"
...
export default {
    ...
    onSubmit(){
      ...
      sendEmail(e) {
        try {
          emailjs.sendForm(emailjskey.servicekey, emailjskey.templatekey_feedback, e.target,emailjskey.userkey, )
