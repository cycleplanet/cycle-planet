  ...
  <div class="bg-white">
    ...
    <q-list v-if="myUserDetails.trips">
      <div v-for="(tripDetails, tripKey) in myUserDetails.trips" :key="tripKey">
        <q-item
          clickable
          v-ripple
          @click="$router.push({ path: '/trip/' + myUserId + '/' + tripKey })"
        >
          <q-item-section>
            ...
            <q-item-label>
              <q-chip
                >{{
                  tripDetails.countries
                    ? Object.keys(tripDetails.countries).length
                    : 0
                }}
                countries</q-chip
              >
              <q-chip
                >{{
                  tripDetails.stories
                    ? Object.keys(tripDetails.stories).length
                    : 0
                }}
                stories</q-chip
              >
    ...
    <div class="justify-center flex items-center q-ma-md q-gutter-x-sm">
      <q-btn
        :style="buttonStyle"
        rounded
        @click="$router.push({ path: '/trips' })"
        >Get inspiration</q-btn
      >
      <div>or</div>
      <q-btn :style="buttonStyle" rounded @click="createTripDialog = true"
        >create new trip</q-btn
      >
    </div>
