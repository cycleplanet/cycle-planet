...
<div>
    ...
    <div v-if="(requestData.receiver===myUserId && requestData.status==='unanswered')||editResponse" class="row">
        <q-item-section>
            <q-input class="bg-white" color="teal" autogrow filled  v-model="response" type="text" label="Response"  lazy-rules/>
        </q-item-section>
        <q-item-section side>
            <q-btn round color="green" icon="done" @click="answer='accept'; (response? answerRequest():noResponse=true)"/>  
        </q-item-section >
        <q-item-section side >
            <q-btn round color="red" type="submitaccept" icon="clear" @click="answer='refuse'; (response? answerRequest():noResponse=true)"/>  
        </q-item-section>
    </div>
    <div class="row" v-if="myUserDetails.hosting.requests[requestkey].receiver===myUserId&&myUserDetails.hosting.requests[requestkey].status!=='unanswered'&&showEdit">
        <q-space/>
    ...
    <q-dialog v-model="noResponse">
        <q-card class="q-pa-md">
            <div>Are you sure you want to {{answer}} without writing a response?</div>
            <div class="row q-mt-md">
                <q-space></q-space>
                <q-btn flat color="teal" @click="noResponse=false">cancel</q-btn>
                <q-btn :style="buttonStyle" @click="answerRequest();noResponse=false">send</q-btn>
            </div>
        </q-card>
    </q-dialog>
