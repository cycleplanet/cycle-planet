  ...
  <div>
      <p class="text-h6">About me</p>
				...
				<q-virtual-scroll virtual-scroll-horizontal :items="[0]">
					<div class="row no-wrap q-gutter-xs">
						<div class="bg-grey-3" v-if="myProfile">
							<q-btn style="margin:50px" outline round icon="add" @click="addPictureDialog=true"/>
						</div>
						<div v-for="(userImage, index) in imageTest" :key="index" >
						<q-img @click="showPictureMethod(index)" style="width:150px;height:150px" :src="userImage" class="cursor-pointer"/>
					</div>
					</div>
				</q-virtual-scroll>
					...
					<div  v-for="(gear, gearKey) in userData.gear_list_new" :key="gearKey" class="q-mt-md">
						...
						<div v-for="(item, itemKey) in gear" :key="itemKey">
							<div v-if="Object.keys(item).length===1">
								<q-item dense >
									<q-item-section>
										<q-item-label >{{itemKey}}: {{Object.values(item)[0]}}</q-item-label>
									</q-item-section>
								</q-item>
							</div>
							<div v-if="Object.keys(item).length>1">
								<div class="q-ml-md">{{itemKey}}:</div>
								<q-item dense v-for="(childitem, childitemKey) in item" :key="childitemKey">
			...
			<div  class="row">
				...
				<q-space></q-space>
				<q-btn round size="sm" v-if="myProfile"  :style="buttonStyle" icon="add"  @click.native="AddPostMethod('Route')"/>
			</div>
			<div class="q-mt-md" >
				<q-virtual-scroll :items="[0]" virtual-scroll-horizontal>
					<div  class="row no-wrap">
						<div v-for="(postItemData, postItemKey, index) in routePostsSorted" :key="index" class="row no-wrap">
			...
			<div class="row">
				...
				<q-space></q-space>
				<q-btn round size="sm" v-if="myProfile"  :style="buttonStyle" icon="add" @click.native="AddPostMethod('Blog')"/>
			</div>
		
			<div class="q-mt-md" v-if="Object.keys(blogPostsSorted).length>0" >
				 <q-virtual-scroll :items="[0]" virtual-scroll-horizontal>
					<div  class="row no-wrap">
						<div v-for="(postItemData, postItemKey, index) in blogPostsSorted" :key="index" class="row no-wrap">
							<div v-if="postItemData.user_created===userId">
								<post-card :postItemData="postItemData"/>
							</div>
						</div>
			...
			<div class="row">
				...
				<q-space></q-space>
				<q-btn round size="sm" v-if="myProfile" :style="buttonStyle" icon="add" @click.native="AddPostMethod('Video')"/>
			</div>
		
			<div class="q-mt-md" v-if="Object.keys(videoPostsSorted).length>0 ">
				 <q-virtual-scroll :items="[0]" virtual-scroll-horizontal>
					<div  class="row no-wrap">
						<div v-for="(postItemData, postItemKey, index) in videoPostsSorted" :key="index" class="row no-wrap">
							<div v-if="postItemData.user_created===userId">
								<post-card :postItemData="postItemData"/>
							</div>
						</div>
	...
	<q-dialog v-model="showPictureDialog" class="absolute-center">
		<q-card style="width:100%;height:auto;" class="absolute-center"> 
			<div class="absolute-left" v-if="myProfile">
				<q-chip label="profile picture" v-if="imageTest[showPicture]===userData.imageurl" class="bg-amber"/>
				<q-chip label="background picture" v-if="imageTest[showPicture]===userData.backgroundimageurl" class="bg-amber"/>
			</div>
			<div class="absolute-left q-ml-lg">

				<q-btn v-if="showPicture>0" flat dense icon="chevron_left" class="bg-white absolute-center" @click="showPicture--"/>

			</div>
			<div class="absolute-right q-ma-sm">
				<q-btn flat dense icon="close" class="bg-white" @click="showPictureDialog=false;showPicture=''"/>
				<q-btn flat dense v-if="(showPicture+1)<Object.keys(imageTest).length" class="bg-white absolute-center" icon="navigate_next" @click="showPicture++"/>
				<q-btn v-if="myProfile" flat  dense icon="delete" class="bg-red text-white absolute-bottom" @click="deleteUserPicture()"/>
			</div>
...
export default {
    ...
    methods:{
		...
		deleteUserPicture(){
			...
			let docName=url2[0]
			firebaseConfig.storage.ref('/users/'+this.userId).child(docName).delete().then(()=>{
