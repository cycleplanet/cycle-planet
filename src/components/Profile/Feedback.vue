<template>
    <div> 
        <div :class="feedback.experience==='Good'?'bg-green-2':(feedback.experience==='Neutral'?'bg-orange-2':'bg-red-2')">
            <q-item>
                <q-item-section class="cursor-pointer" avatar @click="clickuser(otherUserIdFeedback)">
                    <q-avatar size="md" >
                        <img :src="users[otherUserIdFeedback].imageurl">
                    </q-avatar>
                </q-item-section>
                <q-item-section class="text-bold">
                    <q-item-label class="underline-hover cursor-pointer" @click="clickuser(otherUserIdFeedback)">{{users[otherUserIdFeedback].fullname}}</q-item-label>
                    <q-item-label caption>{{feedback.dateProposal}}</q-item-label>
                </q-item-section>
                <q-item-section side>
                    <q-item-label>
                        <q-btn flat v-if="otherUserIdFeedback===myUserId" round dense icon="edit" @click="editFeedbackdialog=true;editfeedback=feedback.feedback"/>
                    </q-item-label>
                </q-item-section>
            </q-item>
            <q-item>
        ...
        <q-dialog v-model="editFeedbackdialog">
            <q-card v-if="editfeedback" style="width:90%; max-width:600px">
                ...
                <div class="row q-mt-sm">
                    <q-space></q-space>
                    <q-btn label="save" :style="buttonStyle" @click="saveEditFeedback()"/>
                </div>
                </div>
            </q-card>
        </q-dialog>
    ...
    methods:{
        saveEditFeedback(){
            ...
            this.updateItemAction({
                path:'Users/'+otheruser+'/hosting/requests/'+this.feedback.timestamp,
                data:{
