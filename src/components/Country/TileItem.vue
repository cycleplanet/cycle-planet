<template>
  <div>
    <div class="q-mb-md" v-if="countryData.name">
      <q-item class="no-padding">
        <q-item-section class="cp-h2"
          ...
          {{ refKey === "Embassy" ? countryData.name : "" }}</q-item-section
        >
        <q-item-section side top>
          <div>
            <q-btn
              ...
              size="sm"
              @click.native="
                loggedIn ? AddItemMethod(refKey) : showLoginDialog()
              "
              v-if="showAddButton"
            />
    ...
    <div v-if="orderMarkers && refKey !== 'Border_item'">
      <q-list separator class="full-width">
        <div v-for="(markerKey, index) in orderMarkers" :key="index">
          <q-item clickable v-if="index < indexSeeDo">
            <q-item-section
              clickable
              v-ripple
              @click="openItem(landMarkers[markerKey])"
            >
              <q-item-label v-if="refKey !== 'Embassy'"
                >{{ index + 1 }}. {{ landMarkers[markerKey].title }}
              </q-item-label>
              <q-item-label v-if="refKey === 'Embassy'" class="row"
                >{{ index + 1 }}.
                <div v-if="landMarkers[markerKey].onlineVisa">Online visa</div>
                <div v-else>
                  {{ landMarkers[markerKey].country_located }} ({{
                    landMarkers[markerKey].city
                  }})
                </div>
              </q-item-label>
              <q-item-label
            ...
            <q-item-section side top>
              <div class="row flex items-center">
                <q-btn
                  ...
                  class="buttontheme2"
                  :label="
                    landMarkers[markerKey].comments
                      ? Object.keys(landMarkers[markerKey].comments).length
                      : 0
                  "
                />
                <like-item
        ...
        <q-item
          clickable
          v-if="
            Object.keys(orderMarkers).length > 5 &&
            Object.keys(orderMarkers).length !== indexSeeDo
          "
          class="row justify-center"
        >
          <div
            class="q-ma-sm text-bold"
            @click="indexSeeDo = Object.keys(orderMarkers).length"
          >
    ...
    <div v-if="orderMarkers && refKey === 'Border_item'" class="cp-p">
      <div
        v-if="
          countryData.borders_new
            ? Object.keys(countryData.borders_new).length
            : false
        "
      >
        <div
          v-for="(neighbourcountry, neighbourcountryKey) in Object.keys(
            countryData.borders_new
          )"
          :key="neighbourcountryKey"
        >
          <div v-if="neighbourcountry !== countryKey" class="q-my-lg">
            ...
            <div v-if="!countryData.borders_new[neighbourcountry]">
              You can cross this border without having to go through customs.{{
                countryData.schengen &&
                allCountryData[neighbourcountry].schengen
              ...
              <q-list class="full-width">
                ...
                <div v-for="(markerKey, index) in orderMarkers" :key="index">
                  <q-item
                    clickable
                    v-if="
                      landMarkers[markerKey].country1.country ===
                        neighbourcountry ||
                      landMarkers[markerKey].country2.country ===
                        neighbourcountry
                    "
                  >
                    <q-item-section
                    ...
                    >
                      <q-item-label class="flex items-center"
                        ...
                        <img
                          ...
                          style="width: 25px;"
                        />{{
                          landMarkers[markerKey].country2.city
                        }}</q-item-label
                      >
                      <q-item-label
                        caption
                    ...
                    <q-item-section side top>
                      <div class="row flex items-center">
                        <q-btn
                          ...
                          :label="
                            landMarkers[markerKey].comments
                              ? Object.keys(landMarkers[markerKey].comments)
                                  .length
                              : 0
                          "
                        />
            ...
            <div v-else class="text-italic">
              You need to pass customs on this border, but there are no points
              added yet.
            </div>
          </div>
        </div>
    ...
    orderMarkers() {
      ...
      allLandMarkers.forEach((element) => {
        ...
        if (itemDetails.refKey === "Border_item") {
          if (
            (itemDetails.country1.country === this.countryKey ||
              itemDetails.country2.country === this.countryKey) &&
            itemDetails.refKey === this.refKey
          ) {
            markerItems.push(itemId);
          }
        } else {
          if (
            itemDetails.countryKey === this.countryKey &&
            itemDetails.refKey === this.refKey
          ) {
            markerItems.push(itemId);
          }
      ...
      markerItems.sort((a, b) => {
        let AProp = this.landMarkers[a].likes
          ? Object.keys(this.landMarkers[a].likes).length
          : 0;
        let BProp = this.landMarkers[b].likes
          ? Object.keys(this.landMarkers[b].likes).length
          : 0;
