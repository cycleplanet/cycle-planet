<template>
    <div>
        <div>
			<q-item class="no-padding">
                ...
                <q-item-section side top>
                    <div>
                    <q-btn :style="buttonStyle" round icon="add" size="sm" @click.native="loggedIn ? AddPostMethod(refKey) : showLoginDialog()"/>
                    <q-btn flat @click.native="loggedIn ? AddPostMethod(refKey) : showLoginDialog()"></q-btn>
                    </div>
        ...
        <div v-if="refKey==='Blog' && blogPostsSorted" >
			<q-virtual-scroll :items="[0]" virtual-scroll-horizontal>
				<div  class="row no-wrap">
					<div v-for="(postItemData, postItemKey) in blogPostsSorted" :key="postItemKey" >
		...
		<div v-if="refKey==='Video' && videoPostsSorted" >
			<q-virtual-scroll :items="[0]" virtual-scroll-horizontal>
				<div  class="row no-wrap">
					<div v-for="(postItemData, postItemKey) in videoPostsSorted" :key="postItemKey">
						<div v-if="postItemData.countries[countryKey]">
							<post-card :postItemData="postItemData"/>
						</div>
					</div>
		...
		<div v-if="refKey==='Route'" >
			<q-virtual-scroll :items="[0]" virtual-scroll-horizontal>
				<div  class="row no-wrap">
            		<div v-for="(postItemData, postItemKey) in routePostsSorted" :key="postItemKey" >
						<div v-if="postItemData.countries[countryKey]">
							<post-card :postItemData="postItemData"/>
						</div>
					</div>
...
export default {
    ...
    computed:{
		...
		...mapGetters('country', ['blogPostsSortedCountry','blogPostsSortedByDateCountry','videoPostsSortedCountry','videoPostsSortedByDateCountry']),
