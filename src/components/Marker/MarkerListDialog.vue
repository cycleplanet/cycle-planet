...
<div>
    <q-card style="height:100%" class="no-padding">
        ...
        <div class="q-ma-sm">
            <download-gpx :markersArray="markersArray"/>
        </div>
        ...
        <q-list bordered class="rounded-borders" v-if="Object.keys(landMarkers).length&&markersArray">
                <div v-for="itemKey in markersArray" :key="itemKey">
                    ...
                    <template v-slot:header>
                        ...
                        <q-item-section>
                            <marker-title class="text-bold" :singleItemData="landMarkers[itemKey]"/>
                        ...
                        <q-item-section avatar v-if="landMarkers[itemKey].user_created===myUserId">
                        <q-avatar :icon="landMarkers[itemKey].status==='approved'?'check_circle':(landMarkers[itemKey].status==='disapproved'?'remove_circle_outline':'pending')" :class="landMarkers[itemKey].status==='approved'?'text-green-10':(landMarkers[itemKey].status==='disapproved'?'text-red-10':'text-orange-10')">
                            <q-tooltip class="bg-primary">
                            <div>{{'Checked '+(landMarkers[itemKey].checked_by?Object.keys(landMarkers[itemKey].checked_by).length:0)+' times out of 3'}}</div>
                            <div >
                                <div class="text-h6 q-mb-sm">Logbook</div>
                                <div class="q-my-md">
                                <nice-date2 class="text-bold" :dateprop="landMarkers[itemKey].date_created"/>
                                <div class="row "><modal-username class="q-mr-sm" :userId="landMarkers[itemKey].user_created"/> created this item</div>
                            </div>
                            <div v-if="landMarkers[itemKey].date_edited" class="q-my-md">
                                <nice-date2 class="text-bold" :dateprop="landMarkers[itemKey].date_edited"/>
                                <div class="row "><modal-username class="q-mr-sm" :userId="landMarkers[itemKey].user_edited"/> edited this item</div>
                            </div>
                            <div v-if="landMarkers[itemKey].logbook">
                                <div v-for="(log,logKey) in landMarkers[itemKey].logbook" :key="logKey" class="q-my-md">
                        ...
                        <q-item-section side>
                            <like-item class="text-black" :singleItemData="landMarkers[itemKey]" />
                        </q-item-section>
                    </template>
        ...
        <div v-if="!markersArray" class="items-center justify-center flex q-mt-lg">
