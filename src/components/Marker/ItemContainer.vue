<template>
  <div class="q-pa-md">
    <div v-if="refKey === 'Embassy'">
      <p class="text-subtitle1" v-if="!singleItemData.onlineVisa">
        Visa for <b>{{ countryKey }}</b> in
        <b>{{ singleItemData.country_located }}</b>
      </p>
      <div v-if="singleItemData.onlineVisa" class="q-mb-md">
        <p class="text-subtitle1">
          E-Visa for <b>{{ singleItemData.country }}</b>
        </p>

        <q-btn
          rounded
          :style="buttonStyle"
          label="Website e-visa"
          @click="openUrl(singleItemData.website)"
        />
      </div>
      <!-- <div v-else>
    ...
    >
      <b>Title:</b> {{ singleItemData.title }}
    </p>
    <p
      class="text-subtitle1 row flex items-center"
      v-if="refKey !== 'Border_item' && refKey !== 'Embassy'"
    >
      <b>Country:</b>
    ...
    <div class="row">
      ...
      <q-space></q-space>
      <div
        class="text-blue-10 underline cursor-pointer"
        v-if="singleItemData.description_translated"
      >
        Translate
        <q-menu>
          <q-list style="min-width: 100px;">
            <q-item
              clickable
              v-close-popup
              v-for="(languageData, languageKey) in translateOptions"
              :key="languageData"
            >
              <q-item-section @click="changeLanguage(languageData.code)">{{
                languageKey
              }}</q-item-section>
            </q-item>
          </q-list>
