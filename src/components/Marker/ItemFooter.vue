    ...
    <div>
        ...
        <q-card-section class="q-pt-none">
            <p class="text-bold cp-h2">Most recent experiences</p>
                <div v-if="singleItemData.comments" class="q-pl-none">
                    <div v-for="(comment, commentId, index) in singleItemData.comments" :key="index">
                            ...
                            <div class="row">
                                <nice-date2 style="font-size:10px" class="text-grey q-ma-sm" :dateprop="comment.date"/>
                                <q-space></q-space>
                                <q-icon name="more_vert" color="grey" size="xs" v-if="loggedIn?(roleEditor||comment.user===myUserId):false">
                                    <q-menu>
                                        <q-list style="min-width: 100px">
                                            <q-item clickable v-close-popup>
                                            <q-item-section @click="editCommentDialog=true;editCommentString=comment">Edit</q-item-section>
                                            </q-item>
                                            <q-item clickable v-close-popup>
                                            <q-item-section @click="deleteCommentMethod(comment.date)">Delete</q-item-section>
                                            </q-item>
                                        </q-list>
                                    </q-menu>
                                </q-icon>
        ...
        <q-dialog v-model="editCommentDialog">
            <q-card>
                ...
                <div class="q-pa-sm">
                    <q-input filled autogrow label="edit comment" v-model="editCommentString.comment"/>
                    <div class="row">
                        <q-space></q-space>
                        <q-btn :style="buttonStyle" size="sm" class="q-mt-sm" @click="editCommentMethod(editCommentString)">Sumbit</q-btn>
                    </div>
                </div>
            </q-card>
        </q-dialog>
     ...
     components:{
      ...
      methods:{
        editCommentMethod(comment){
            ...
            firebaseConfig.fs.collection('Markers').doc(this.itemKey).update({comments:this.singleItemData.comments})
