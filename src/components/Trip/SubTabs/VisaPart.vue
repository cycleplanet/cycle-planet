      ...
      <div v-for="(country, index) in mySelectedCountriesComputed" :key="index">
        <div class="q-my-md spacing">
          ...
          <div class="border">
            ...
            <!-- {{checkVisaData(index)}} -->
            <div
              v-if="allCountryData[mySelectedCountriesComputed[index]].schengen"
            >
            ...
            <div v-else>
              <div v-if="visaData[mySelectedCountriesComputed[index]]">
                <div
                  v-html="visaData[mySelectedCountriesComputed[index]].body"
                ></div>
              </div>
              <div v-else>
                We don't have experience with getting this visa. Please share
                yours if you have recent experience.
              </div>
            </div>
          </div>
          <div v-if="checkEVisaData(index)">
            <div
              v-for="(embassy, itemKey) in markerToApply[country]"
              :key="itemKey"
            >
          ...
          <div v-else-if="checkVisaData(index) === 'second'">
            ...
            >
              <div
                v-if="
                  embassy.country_located ===
                  mySelectedCountriesComputed[index - 2]
                "
              >
                <visacard :countryKey="country" :itemKey="itemKey" />
              </div>
          ...
          <div v-else-if="checkVisaData(index) === 'third'">
            ...
            >
              <div
                v-if="
                  embassy.country_located ===
                  mySelectedCountriesComputed[index - 3]
                "
              >
                <visacard :countryKey="country" :itemKey="itemKey" />
              </div>
    ...
    checkEVisaData(index) {
      ...
      if (this.validmarkers[this.mySelectedCountriesComputed[index]]) {
        let keys = Object.keys(
          this.markerToApply[this.mySelectedCountriesComputed[index]]
        );
        keys.forEach((element) => {
          if (
            this.markerToApply[this.mySelectedCountriesComputed[index]][element]
              .onlineVisa
          ) {
            x = x + 1000;
          }
      ...
      if (this.markerToApply[this.mySelectedCountriesComputed[index]]) {
        ...
        keys.forEach((element) => {
          if (
            ...
            this.markerToApply[this.mySelectedCountriesComputed[index]][element]
              .country_located === this.mySelectedCountriesComputed[index - 1]
          ) {
            x = x + 1000;
          } else if (
            this.markerToApply[this.mySelectedCountriesComputed[index]][element]
              .country_located === this.mySelectedCountriesComputed[index - 2]
          ) {
            x = x + 100;
          } else if (
            this.markerToApply[this.mySelectedCountriesComputed[index]][element]
              .country_located === this.mySelectedCountriesComputed[index - 3]
          ) {
            x = x + 10;
          }
        });
