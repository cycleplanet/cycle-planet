<template>
  <div>
    <div
      v-if="tripActive && myProfile && storyData.country"
      class="q-pa-md q-mb-lg"
      style="max-width: 450px;"
    >
      <div class="row" :class="isWebApp ? 'cp-h2 ' : 'text-h6'">
        {{ storyData.country }}, {{ storyData.place }}
        <div class="circle pulse q-ma-sm"></div>
      </div>
      <q-btn :style="buttonStyle" @click="addStoryDialog = true"
        >Add story</q-btn
      >
    </div>
      ...
      >
        <div class="q-pa-md">
          <q-btn
            ...
            round
            icon="edit"
            @click="editStoryMethod(story, storyId)"
            class="absolute-top-right q-pa-sm"
            v-if="myProfile"
          />

          <div class="no-margin" :class="isWebApp ? 'cp-h2' : 'text-h6'">
            {{ story.title }}
          </div>
    ...
    <div v-else>
      <div v-if="myProfile">
        ...
        <div v-else>
          You have no stories yet. You can add your first story when your trip
          started.
        </div>
      </div>

      <div v-else>{{ users[userId].fullname }} has no stories yet.</div>
    </div>

    <q-dialog
      :maximized="!isWebApp"
      v-model="addStoryDialog"
      @close="addStoryDialog = false"
    >
      <q-card style="width: 80%; max-width: 500px;" class="no-padding">
        ...
        <div class="q-pa-md q-gutter-y-sm">
          ...
          <div class="text-italic">
            {{ storyData.coordinates.lat }}, {{ storyData.coordinates.lng }}
          </div>
    ...
    >
      <q-card style="min-width: 40%;" class="" v-if="editStoryData.story">
        ...
        <div class="q-pa-md q-gutter-y-sm">
          <div class="cp-h2">
            {{ editStoryData.story.country }}, {{ editStoryData.story.place }}
          </div>
          <div class="text-italic">
            {{ editStoryData.story.coordinates.lat }},
            {{ editStoryData.story.coordinates.lng }}
          </div>
          <q-input
            label="Title"
            filled
            v-model="editStoryData.story.title"
          ></q-input>
          <div class="text-bold">Description:</div>
