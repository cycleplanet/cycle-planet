...
<div>
  <div v-if="!tripDetails.countries">
    <div v-if="myProfile">
      <q-select outlined filled v-model="newTrip.countries" :options="countryKeys" label="Select your start country" class="q-mb-md bg-white"/>
            ...
            <div  :style="screenwidthbig?'padding:2px':'padding:1px'" >
              <q-chip   :size="screenwidthbig?'15px':'12px'"  clickable outline class="text-subtitle1 " >
                <q-avatar rounded style="width:auto;" class="" clickable @click="clickedcountryMethod(indexBorder)">
                  <img style="border:1px solid black; margin-left:-1px" :src="('countryflags/Flag_of_'+indexBorder+'.svg.png').split(' ').join('_')" >
                </q-avatar>
  ...
  <q-dialog v-model="selectCountryDialog">
    <q-card class="no-padding" style="width:40%;max-width:400px">
      ...
      <div class="q-pa-md">
        <q-select outlined v-model="countrySelected" :options="countryKeys" label="Select country" class="q-mb-md bg-white"/>
