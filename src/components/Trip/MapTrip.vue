  ...
  <div style="height: 100%;" v-if="users && tripDetails && allCountryData">
    <div style="height: 100%;">
        ...
        :center="
          allCountryData[
            tripDetails.countries[
              Math.floor(Object.keys(tripDetails.countries).length / 2)
            ]
          ].location
        "
      ...
      >
        ...
        <div v-if="!tripFinished">
          ...
          >
            <l-marker
              v-if="showParts"
              :lat-lng="[
                allCountryData[country].location.lat,
                allCountryData[country].location.lng,
              ]"
            >
              <l-icon
                :class-name="
                  'markerStyleCountry markerStyle-z' + zoomStyle[zoom]
                "
              >
            ...
            <l-polyline
              ...
              dashOffset="3"
              :lat-lngs="[
                allCountryData[tripDetails.countries[countryKey - 1]].location,
                allCountryData[tripDetails.countries[countryKey]].location,
              ]"
            ></l-polyline>
          </div>
        </div>
        ...
        <div v-if="!tripFuture">
          <div
            v-for="(story, storyId, index) in tripDetails.stories"
            :key="storyId"
          >
            <l-marker :lat-lng="story.coordinates">
              <l-icon
                :class-name="'markerStyleStory markerStyle-z' + zoomStyle[zoom]"
              >
            ...
            <l-polyline
              v-if="index > 0"
              :lat-lngs="[
                tripDetails.stories[storyKeys[index - 1]].coordinates,
                tripDetails.stories[storyKeys[index]].coordinates,
              ]"
              color="rgb(252,186,3)"
            />
          </div>
        ...
        <div v-if="storyData.coordinates && tripActive">
          ...
          <l-polyline
            v-if="tripDetails.stories"
            color="red"
            dashArray="20,20"
            dashOffset="3"
            :lat-lngs="[
              tripDetails.stories[
                storyKeys[Object.keys(tripDetails.stories).length - 1]
              ].coordinates,
        ...
        <div v-if="!tripFinished && myProfile">
          <div
            v-for="(marker, markeryKey) in tripDetails.markers"
            :key="markeryKey"
          >
            <l-marker
              v-if="showParts"
              :lat-lng="landMarkers[marker.itemKey].coordinates"
            >
              <l-icon
        ...
        <div v-if="!showParts">
          <l-control
            position="topleft"
            class="map-overlay no-margin cursor-pointer"
          />
          ...
          <l-control
            position="bottomleft"
            class="q-gutter-xs q-ma-md ownfont text-white"
          >
            <div class="text-h4 q-ma-sm cursor-pointer">
              {{ tripDetails.title }}
            </div>
            <div class="row q-gutter-x-sm text-center">
              <div v-if="tripFuture" class="col" style="width: 70px;">
                ...
                <div>
                  {{ daysToGo(tripDetails.startdate) === 1 ? "Day" : "Days" }}
                  to go
                </div>
              </div>
              <div v-if="!tripFuture" class="col" style="width: 70px;">
