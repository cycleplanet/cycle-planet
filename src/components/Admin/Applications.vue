            ...
            <q-item>
              <q-item-section>
                <q-item-label class="text-h6">{{
                  item.user ? users[item.user].fullname : "no userid"
                }}</q-item-label>
              ...
              <q-item-section side top>
                ...
                <q-btn
                  dense
                  flat
                  @click="acceptEditorAction(itemKey)"
                  icon="done"
                  label="accept"
                  class="bg-green-2 text-green q-ma-xs"
                />
                <q-btn
                  dense
                  flat
                  @click="rejectEditorAction(itemKey)"
                  icon="delete"
                  label="reject"
                  class="bg-red-2 text-red q-ma-xs"
                />
              </q-item-section>
            </q-item>
          </q-list>
...
export default {
  ...
  methods: {
    ...
    rejectEditorAction(itemKey) {
      ...
      let message =
        "Hello, we just reviewed your application and we want to let you know that we rejected your application. Unfortunately we're not convinced (yet) that you can contribute useful information for our platform. Feel free to ask us for more explaination, and perhaps we can give it another try. Best, the Cycle Planet team";
      this.setItemAction({
        path: "Chats/" + this.myUserId + "/" + data.user + "/" + this.timeStamp,
        data: {
          text: message,
