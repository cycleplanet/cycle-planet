            ...
            <q-item>
              <q-item-section>
                ...
                <q-item-label caption>{{
                  item.user ? users[item.user].fullname : "no user id"
                }}</q-item-label>
                <q-item-label v-if="item.platformdata">
                  <div v-for="(value, key) in item.platformdata" :key="key">
                    <b>{{ key }}:</b> {{ value }}
                  </div>
                </q-item-label>
              </q-item-section>
              <q-item-section side top>
                <div class="row">
                  <q-btn
                    ...
                    dense
                    flat
                    @click="doneFeedbackAction({ key: itemKey })"
                    icon="done"
                  />
                  <q-btn
                    v-if="item.status"
                    dense
                    flat
                    @click="deleteFeedbackAction({ key: itemKey })"
                    icon="delete"
                  />
                  <q-btn
                    flat
