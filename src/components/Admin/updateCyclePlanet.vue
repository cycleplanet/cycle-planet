<template>
  <div>
    <q-btn @click="updateCyclePlanet()" :style="buttonStyle">Update Cycle Planet</q-btn>
        ...
        <div class="bg-white" style="width:400px">
          <q-btn @click="startBackup">Update markers</q-btn>
            <div>
              ...
              <q-linear-progress v-if="backupMarkersStatus" :value="progress" class="q-my-md" />
            </div>
            <div>
              <div >Assign markers to users ({{loadingLength}}) - {{currentItem}}</div>
              <q-linear-progress v-if="assignMarkersStatus" :value="progress" class="q-my-md" />
            </div>
        </div>
     ...
     methods:{
        ...
        assignMarkers(){
          ...
          markerKeys.forEach(element => {
            ...
            this.currentItem=this.landMarkers[element].title
            this.updateItemAction({
              path:'Users/'+this.landMarkers[element].user_created+'/points/markers_added',
              data:{
          ...
          userKeys.forEach(element => {
            if(this.users[element].points){
              if(this.users[element].points.markers_added){
                let totalMarkersAdded=Object.keys(this.users[element].points.markers_added).length
