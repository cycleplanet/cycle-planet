    ...
    <form @submit.prevent="sendMessage">
      ...
      <q-card-section class="">
        <q-banner inline-actions class="text-white bg-red" v-if="otherStatus">
          ...
          <div
            v-if="users[otherUserId].hosting.status !== 'Available for hosting'"
          >
            This user is {{ users[otherUserId].hosting.status }}
          </div>
        </q-banner>

        <p>
          By sending a host request, an email will be send to the other user.
          The message will also appear in the chat.
        </p>
        <q-input
          dense
          v-model="newMessage"
          ...
          color="teal"
        />
        <modal-due-date class="q-mt-md" :dueDate.sync="dateProposal" />
    ...
    sendEmail(e) {
      try {
        emailjs.sendForm(
          ...
          emailjskey.templatekey_hostrequest,
          e.target,
          emailjskey.userkey
        );
      } catch (error) {}
