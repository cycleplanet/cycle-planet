<template>
  <div
    :style="messageItem.from === 'me' ? 'margin-left:30%' : 'margin-right:30%'"
  >
    <q-chat-message
      :text="message"
      :sent="messageItem.from === 'me' ? true : false"
      :bg-color="
        messageItem.type === 'request'
          ? 'amber'
          : messageItem.from == 'me'
          ? 'white'
          : 'light-green-2'
      ...
      <div v-if="messageItem.type === 'request'">
        <div v-if="messageItem.from === 'them'">
          <div v-if="messageItem.status === 'unanswered'">
            <modal-response
              :requestkey="messageItem.timestamp"
              :showEdit="false"
            />
          </div>
...
export default {
  ...
  components: {
    "modal-response": require("components/Profile/Requests/ModalResponse.vue")
      .default,
  },
    ...
    message() {
      if (this.messageItem.type === "request") {
        if (this.messageItem.status === "deleted") {
          ...
          return [
            "HOST REQUEST: " + this.messageItem.text,
            "Date: " + this.messageItem.dateProposal,
          ];
        }
      } else {
