<template>
  	<q-page class="bg-grey-3" >
	  	...
	  	<q-toolbar class="bg-secondary" :class="screenwidthbig?(flexlist?'flexwidth':'solidwidth'):'mobilewidth'">
			...
			<q-btn flat round icon="more_vert">
				<q-menu anchor="top right" self="top left">
					<q-item clickable v-if="checkIfRequestWasSendRecently===0">
						<q-item-section @click="hostRequestMethod">Send host request</q-item-section>
					</q-item>
					<q-item clickable v-if="users[userId].coordinates">
						<q-item-section @click="isWebApp?openUrl('https://www.openstreetmap.org/#map=19/'+users[userId].coordinates.lat+'/'+users[userId].coordinates.lng):openMapUrl(users[userId].coordinates.lat,users[userId].coordinates.lng)">Get location</q-item-section>
					</q-item>
					<q-item clickable v-if="users[userId].phonenumber">
						<q-item-section @click="clickPhonenumber">{{users[userId].phonenumber}}</q-item-section>
					</q-item>
				</q-menu>
			</q-btn>
		</q-toolbar>
		<q-toolbar class="bg-white bottom0" :class="screenwidthbig?(flexlist?'flexwidth':'solidwidth'):'mobilewidth'">
			<q-form class="full-width">

				<q-input v-if="isWebApp" v-model="newMessage" bg-color="white" ref="newMessage" outlined autogrow rounded label="Message" dense>
				<template v-slot:after>
					<q-btn flat round dense @click="sendMessage" type="submit" icon="send" />
				</template>
				</q-input>
				<q-input v-if="!isWebApp" v-model="newMessage" bg-color="white" ref="newMessage" outlined autogrow rounded label="Message" dense>
				<template v-slot:after>
	...
	<div class="q-pa-sm" style="padding-bottom:50px">
		<q-page-container ref="pageChat" >
			...
			<div v-else class="justify-center flex" @click="hostRequestDialog=true">
...
import FCM from'cordova-plugin-fcm-with-dependecy-updated'
	...
	computed: {
		...
		checkIfRequestWasSendRecently(){
			...
			if(this.users[this.myUserId].hosting.requests){
				let myRequests = Object.keys(this.users[this.myUserId].hosting.requests)
				myRequests.forEach(element => {
					if(this.users[this.myUserId].hosting.requests[element].receiver===this.userId){
						x=10
					}
				})
			}
	...
	methods: {
		...mapActions('chat',['firebaseGetMessages','firebaseReadMessages', 'firebaseStopGettingMessages','firebaseSendMessage']),
	  	sendMessage() {
		 ...
		 clickPhonenumber(){
			 this.openUrl('https://api.whatsapp.com/send?phone='+this.users[this.userId].phonenumber);
		 },
		 clearMessage(){
			 this.newMessage=''
