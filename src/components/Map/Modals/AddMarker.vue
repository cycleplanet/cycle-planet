  ...
  <q-card class="no-padding" style="width: 100%;">
    ...
    <q-form @submit.prevent="submitMarker()" ref="myForm">
      <q-card-section class="q-gutter-y-md">
        ...
        <p class="text-h6" v-if="countryKey">
          <b>Country:</b> {{ countryKey }}
        </p>
        <div class="text-h6" v-if="!countryKey">
          <b>Country:</b> {{ payload.country }}{{ currentLocation }}
        </div>
        <q-input
          filled
          v-model="payload.title"
          label="Title activity"
          :rules="[(val) => val.length >= 5 || 'Use at least 5 characters']"
        />
    ...
    errorCountry() {
      this.$q
        .dialog({
          ...
          message:
            "With these coordinates, we can`t reckognize a country. Please try different coordinates.",
        })
        .onOk(() => {});
    },
