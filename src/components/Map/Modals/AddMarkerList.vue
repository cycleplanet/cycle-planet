<template>
    <div>
        <q-card class=" no-padding" :class="isWebApp?'q-card-list':''" v-show='!showAddMarker'>
            <q-list bordered>
                ...
                <div v-for="(item, key) in markerlist" :key="key" clickable>
                    <q-item v-ripple v-if="!item.hideInList" :class="item.markertint" class="q-pa-none cursor-pointer" @click.native="addMarker(key)">
                        <q-item-section class="content-center q-pa-none" :class="item.markercolor" avatar>
            ...
            <div v-if="isWebApp && (myUserId==='xKwHkNyps6h5esaIrzUwjsMsXX82'||admin)" @click="bulkDialogMethod" class="cursor-pointer q-pa-md text-blue justify-center flex">Add bulk</div>
        </q-card>
                ...
                <div class="q-pa-sm">
                    ...
                    <q-stepper v-model="step" ref="stepper" color="secondary" animated>
                        <q-step :name="1" title="Import data" :done="step > 1">
                            ...
                            <div class="">- Enter 4 values per row separated with a semi column and space (; ) and closed with a hashtag (#) without spaces</div>
                            <div>- You can't add border crossings and embassies with the bulk upload</div>
                            <div>- If something goes wrong, please reset the page and try again</div>
                            <div>- This is the very first version of Bulk upload (thanks to Pedro!), so things might brake. Contact Bas if something went wrong.</div>

                            <div class="q-my-sm text-h6">Example:</div>
                            <div class="text-h6">Title; description; latitude; longitude#</div>
                            <div class="bg-grey-4 q-pa-sm text-h6">
                                <div>Taj Mahal; A majestic place in India; 27.17384; 78.04313#</div>
                                <div>Santiago de Compostella; A majestic place in Spain; 42.8804219; -8.5458608#</div>
                            </div>

                            <modal-description :description.sync="description" class="q-mt-md" />

                            <div class="row q-my-md">
                                <q-space/>
                                <q-btn label="format" :style="buttonStyle" class="justify-end " @click="formatText()" />
                            </div>
                        </q-step>

                        <q-step :name="2" title="Check data" icon="create_new_folder" :done="step > 2">

                            <q-btn v-if="Object.keys(bulkMarkers).length>0?bulkMarkers[Object.keys(bulkMarkers)[Object.keys(bulkMarkers).length-1]].refKey:false" label="submit" :style="buttonStyle" @click="submitBulkMarkers" class="q-my-md" />
                            <div v-if="Object.keys(bulkMarkers).length>0?!bulkMarkers[Object.keys(bulkMarkers)[Object.keys(bulkMarkers).length-1]].refKey:false" class="text-red">Please fill in all categories first, then you can submit the markers.</div>
                            ...
                            <div class="row flex items-start q-my-md" v-for="(markerRow, markerRowId) in bulkMarkers" :key="markerRowId">

                                <q-select outlined :options="Object.keys(markerOptions)" v-model="markerRow.refKey" class="col-2" />
                                <div class="col-1">
                                    {{markerRow.countryKey}}
                                </div>

                                <q-input outlined label="Title" v-model="markerRow.title" class="col-2" />
                                <q-input outlined label="Description" v-model="markerRow.description" type="textarea" class="col-5" />
                                <q-input outlined label="Latitude" v-model="markerRow.coordinates.lat" class="col-1" />
                                <q-input outlined label="Longitude" v-model="markerRow.coordinates.lng" class="col-1" />
                            </div>
                        </q-step>
                    </q-stepper>
                </div>
        ...
        <q-dialog :maximized="!isWebApp" v-model="addEmbassyItemDialog">
            <add-marker-embassy @close="addEmbassyItemDialog = false" :refKey="clickedRef" />
        </q-dialog>

        <q-dialog :maximized="!isWebApp" v-model="addBorderItemDialog">
            <add-marker-border @close="addBorderItemDialog = false" :refKey="clickedRef" />
        </q-dialog>
...
export default {
    ...
    methods: {
        ...
        addMarker(id) {
            this.clickedRef = id
            if (id === 'Embassy') { this.addEmbassyItemDialog = true } else if (id === 'Border_item') { this.addBorderItemDialog = true }
            // else if(id==='SeeDo'){this.addSeeDoItemDialog=true}
            else { this.showAddMarker = true }
        },
