    ...
    <q-form @submit.prevent="submitMarker()" ref="myForm">
      <q-card-section class="q-gutter-y-md">
        <q-banner dense rounded class="bg-green-2 text-green-10" v-if="canEdit"
          >You can edit the <b>category, title</b> and
          <b>coordinates</b> because you
          {{
            admin
              ? "are an admin."
              : roleEditor
        ...
        <q-banner
          ...
          v-if="!canEdit"
          >To edit categrory/country/title/coordinates you need to be an
        ...
        <div v-if="canEdit" class="q-gutter-y-md">
          ...
          <q-btn
            label="update map"
            @click="updateCoords()"
            :class="mapUpdated ? 'buttontheme2' : 'buttontheme1'"
          />
          <q-banner
            inline-actions
            rounded
  ...
  computed: {
    ...
    canEdit() {
      if (
        this.admin ||
        this.roleEditor ||
        this.singleItemData.user_created === this.myUserId
      ) {
        return true;
      } else {
        return false;
