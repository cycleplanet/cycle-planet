		...
		<modal-header>Edit Border Marker</modal-header>
			<q-form @submit.prevent.stop="submitMarker()" ref="myForm">
				<q-card-section class="q-gutter-y-md">
					...
					<q-banner dense rounded class="bg-green-2 text-green-10" v-if="canEdit">You can edit the <b>cities</b> and <b>coordinates</b> because you {{admin?'are an admin.':(roleEditor?'are an editor.':(data.user_created===myUserId?'created this item.':''))}}</q-banner>

					<q-banner inline-actions rounded class="bg-orange text-white" v-if="!canEdit">To edit country/title/coordinates you need to be an <span class="underline cursor-pointer" @click="applyEditor=true">editor</span></q-banner>



					<div v-if="canEdit" class="q-gutter-md">
						<div class="text-bold">From:</div>
						<q-select label="Country" disabled v-model="data.country1.country" filled />
						<q-input label="City" v-model="data.country1.city" filled/>
						<div class="text-bold">To:</div>
						<q-select label="Country" disabled v-model="data.country2.country" filled />
					...
					<div v-else>
						<p><b>From:</b> {{data.country1.country}} ({{data.country1.city}})</p>
						<p><b>To:</b> {{data.country2.country}} ({{data.country2.city}})</p>
