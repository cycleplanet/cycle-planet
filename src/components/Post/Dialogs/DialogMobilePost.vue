   ...
   <q-layout view="Lhh lpR fff" container class="bg-white">
        ...
        <q-page-container class="q-pa-sm" v-if="postItemData">
            ...
            <div v-if="postItemData.comments">
                <div v-for="(comment, commentKey) in postItemData.comments" :key="commentKey" class="q-mt-sm">
                    <div class="row">
                        <div class="col-2">
                            <q-avatar size="md" class="cursor-pointer q-mt-xs" @click="clickuser(comment.user)">
                                <img :src="users[comment.user].imageurl"  >
                            </q-avatar>
                        </div>
                        <div class="col">
                            <div><b class="q-mr-sm cursor-pointer" @click="clickuser(comment.user)">{{users[comment.user].fullname}}</b>{{comment.comment}}</div>
          ...
          <q-footer class="bg-white q-pa-sm text-black">
               <comment v-if="loggedIn" class=" full-width"
