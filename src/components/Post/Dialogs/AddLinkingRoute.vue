<template>
  <div>
    <q-card class="bg-white">
      ...
      <div class="q-ma-sm text-h6" v-if="!selectedCollection">
        Select collection
      </div>
      <div
        class="q-ma-sm text-h6 row items-center cursor-pointer"
          ...
          <div v-for="collection in postItemData.collections" :key="routeItem">
            <q-item clickable v-ripple @click="selectedCollection = collection">
              <q-item-section>
                ...
                <q-item-label
                  >{{
                    Object.keys(routeCollectionData[collection].routes).length
                  }}
                  routes</q-item-label
                >
              </q-item-section>
          ...
          <q-list>
            ...
            >
              <q-item>
                <q-item-section
                  >{{ index + 1 }}.
                  {{ routePostsSorted[routeKey].title }}</q-item-section
                >
                <q-item-section side>
                  <q-btn
                    dense
                  ...
                  <q-btn
                    ...
                    v-if="
                      index + 1 !==
                      Object.keys(
                        routeCollectionData[selectedCollection].routes
                      ).length
                    "
                    icon="expand_more"
                  ></q-btn>
    ...
    <q-dialog v-model="addRouteDialog" :maximized="true">
      <q-card class="bg-white">
        ...
        <q-list>
          <div
            v-for="(routeItem, routeItemKey) in routePostsFiltered"
            :key="routeItemKey"
          >
            <div
              v-if="showMyRoutes ? routeItem.user_created === myUserId : true"
            >
              <q-item clickable v-ripple>
                <q-item-section>
                  <q-item-label
                    ><b>{{ routeItem.title }}</b></q-item-label
                  >
